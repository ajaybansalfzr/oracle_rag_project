...
from helpers.helpers_v6_7 import (
    log_info, save_to_csv, analyze_and_classify_styles,
    convert_block_to_markdown, extract_hyperlinks
)
...
for d in (DST_DIR, OUTPUT_DIR): d.mkdir(exist_ok=True)
    ...
    for path in pdf_files:
        ...
        try:
            with fitz.open(path) as doc:
                ...
                if not style_map:
                    log_info(name, "Could not generate a style map. Aborting extraction for this file.")
                    continue
                        ...
                        if block.get("type") == 0: # It's a text block
                            ...
                            if is_new_section and current_section_content:
                                ...
                                all_chunks.append({
                                    ...
                                    "Document_Name": name, "Chapter": current_h1,
                                    "Section": current_h2, "SubSection": current_h3,
                                    "Content_Markdown": "\n".join(current_section_content),
                                    "Hyperlinks": hyperlinks
                                })
                                current_section_content = []
                                chunk_id_counter += 1
                                if tag == "h1": current_h2, current_h3 = "", ""
                                if tag == "h2": current_h3 = ""
                            
                            # Update current heading state and add heading to content
                            if tag == "h1": current_h1 = markdown_text.replace("# ", "")
                            elif tag == "h2": current_h2 = markdown_text.replace("## ", "")
                            elif tag == "h3": current_h3 = markdown_text.replace("### ", "")
