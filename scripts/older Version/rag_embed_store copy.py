...
def text_to_token_aware_chunks(text: str, chunk_size: int, overlap: int) -> list[str]:
    ...
    if len(tokens) <= chunk_size: return [tokenizer.decode(tokens, skip_special_tokens=True)]
    chunks = []
...
def main():
    ...
    for csv_file in csv_files:
        ...
        if not doc_name_match: continue
        ...
        for _, row in df.iterrows():
            ...
            for chunk in text_chunks:
                ...
                """, (
                    vector_id_counter, chunk_id, doc_name, int(row['page_num']),
                    row['section_id'], chunk, row['header_text'], row['footer_text'],
