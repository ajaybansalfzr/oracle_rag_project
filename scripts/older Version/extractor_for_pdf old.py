...
from scripts.utils.utils import (
    # save_to_csv, is no longer needed
    get_logger,
    clean_text,
...
# NEW: Initialize logger for this script
...
def main():
    ...
    if not pdf_files:
        ...
        logger.warning(f"No PDF files found in '{SRC_DIR}'. Please add PDFs to process.")
        return
    ...
    for pdf_path in pdf_files:
        ...
        logger.info(f"Starting extraction for: {doc_name}")
        ...
        try:
            ...
            if existing_doc:
                if existing_doc['file_hash'] == file_hash:
                    ...
                    # Move to processed even if skipped, to clean up the 'data' folder
                    ...
                    # --- NEW STEP (CR-004): Explicitly delete from vector stores FIRST ---
                    # We pass the old_doc_id to the new utility function
                    # It's crucial to do this BEFORE deleting from the main DB, as the function
                    # needs to look up the chunk_ids associated with the old doc_id.
                    remove_document_vectors(old_doc_id)

                    # Get chunk_ids to be deleted for logging purposes (optional but good)
                    cursor.execute("""
                        SELECT c.chunk_id FROM chunks c
                        JOIN sections s ON c.section_id = s.section_id
                        WHERE s.doc_id = ?
                    """, (old_doc_id,))
                    chunks_deleted = len(cursor.fetchall())                  


                    # Granularly delete old data: chunks, then sections, then the document entry
                    
                    cursor.execute("DELETE FROM chunks WHERE section_id IN (SELECT section_id FROM sections WHERE doc_id = ?)", (old_doc_id,))
                    ...
                    logger.info(f"Removed old doc_id {old_doc_id}, {chunks_deleted} associated chunks, and all sections. Proceeding with new version.")
            ...
            with fitz.open(pdf_path) as doc:
                ...
                logger.info("Pass 1/3: Analyzing font structure and extracting raw blocks...")
                    ...
                    if not body_blocks and Image and pytesseract:
                        ...
                        try:
                            ...
                            if ocr_text.strip():
                                ...
                                logger.info(f"OCR for page {page.number + 1} yielded no text.")
                            continue # Skip the normal block processing for this page
                        except Exception as ocr_error:
                            # print(f"  -  OCR failed for page {page.number + 1}: {ocr_error}")
                            logger.error(f"OCR failed for page {page.number + 1}: {ocr_error}")
                            continue
                ...
                logger.info("Pass 2/3: Building section IDs and extracting page enrichments...")
            ...
            logger.info("Pass 3/3: Assembling and saving final structured data...")
            ...
            if df_final.empty:
                ...
                logger.warning(f"No text could be extracted from '{doc_name}' after processing. Skipping.")
                continue
            ...
            logger.info(f"Successfully saved {len(df_final)} sections for '{doc_name}' to the database.")
            successfully_processed_paths.append(pdf_path)
