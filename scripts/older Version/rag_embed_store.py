...
def main():
    ...
    for model_name in EMBEDDING_MODELS:
        ...
        for path in [config["faiss_path"], config["metadata_path"], config["bm25_path"]]:
        ...
        if not summaries_to_embed:
            print(f"  -  No new summaries to embed for this model.")
            conn_main.close()
            continue
        ...
        cursor_meta.executemany(
            ...
            [(r['chunk_id'], r['doc_name'], r['page_num'], r['section_header'], r['summary'], r['chunk_text']) for r in all_metadata]
        )
        conn_meta.commit()
        conn_meta.close()
        ...
        cursor_main.executemany(f"UPDATE chunks SET {config['vector_id_col']} = ? WHERE chunk_id = ?", update_data)
        conn_main.commit()
