...
def expand_query(query):
    synonyms = {
        "BI tools": ["business intelligence tools", "analysis tools", "report types", "reporting tools"],
...
def ask_llama3_thinking(primary, secondary, question, persona="consultant", review=False):
    ...
    secondary_blocks = [f"Secondary {i+1}:\n{r['Paragraph_Synopsis'] or r['Header'] or r['Footer']}" for i, (_, r) in enumerate(secondary)]
...
def main():
    ...
    parser.add_argument("--persona", type=str, default="consultant", help="Persona: consultant, developer, or user")
