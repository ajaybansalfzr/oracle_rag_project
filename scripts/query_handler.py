...
import sys
import time
from typing import List, Tuple  # #, Dict, Any # Add this to your imports at the top
...
def load_resources():
    ...
    try:
        ...
        for model_name in config.EMBEDDING_MODELS_LIST:
            ...
            MODELS[model_name]["chunk_faiss"] = faiss.read_index(
                str(config.EMBEDDINGS_DIR / f"chunk_index_{safe_name}.faiss")
            )
            ...
            MODELS[model_name][
                "chunk_meta_path"
            ] = f"file:{config.EMBEDDINGS_DIR / f'chunk_metadata_{safe_name}.db'}?mode=ro"
            # --- END CORRECTION ---

            # Load Tier 2 (Section) Artifacts
            MODELS[model_name]["section_faiss"] = faiss.read_index(
                str(config.EMBEDDINGS_DIR / f"section_index_{safe_name}.faiss")
            )
            ...
            MODELS[model_name][
                "section_meta_path"
            ] = f"file:{config.EMBEDDINGS_DIR / f'section_metadata_{safe_name}.db'}?mode=ro"
            # --- END CORRECTION ---
...
def hierarchical_search(query: str, ensemble: bool = True) -> List[int]:
    ...
    top_section_ids_unfiltered = sorted(fused_section_scores, key=fused_section_scores.get, reverse=True)[
        : config.HIERARCHICAL_SEARCH_TOP_N_SECTIONS
    ]
    ...
    with get_db_connection() as main_conn:
        ...
        top_section_ids = [
            row["section_id"]
            for row in main_conn.execute(active_section_ids_query, top_section_ids_unfiltered).fetchall()
        ]
    ...
    candidate_chunk_ids = {
        ...
        for row in main_conn.execute(
            f"SELECT chunk_id FROM chunks WHERE section_id IN ({placeholders})",
            top_section_ids,
        )
    }
    # main_conn.close()
    ...
    for model_name in models_to_search:
        ...
        candidate_bm25_scores = {
            bm25_doc_map[i]["id"]: all_bm25_scores[i]
...
def rerank_and_format_context(query: str, chunk_ids: List[int], max_tokens: int) -> Tuple[str, List[str]]:
    ...
    with sqlite3.connect(meta_path, uri=True) as meta_conn:
        ...
        query_text = f"SELECT chunk_id, raw_text FROM metadata WHERE chunk_id IN ({placeholders})"
        id_to_text_map = {
            row["chunk_id"]: row["raw_text"] for row in meta_conn.execute(query_text, tuple(chunk_ids)).fetchall()
        }
    ...
    for row in final_rows:
        ...
        if not section_id:
            ...
            res = main_db_conn.execute(
                "SELECT section_id FROM chunks WHERE chunk_id = ?", (row["chunk_id"],)
            ).fetchone()
            if res:
...
def query_rag_pipeline(query: str, chat_history: list, persona: str, ensemble: bool) -> Tuple[str, List[str]]:
    ...
    if not context:
        logger.warning("No relevant context found after reranking.")
        return (
            "I found some potentially relevant documents, but could not extract a clear answer after re-ranking.",
            [],
        )
    ...
    if not thought_process or "[LLM Error" in thought_process:
        logger.error("Failed to generate a thought process. Aborting final answer generation.")
        return (
            "I apologize, but I encountered an error while trying to reason about your question. Please try again.",
            [],
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_ensemble",
        action="store_true",
        help="Use only the primary embedding model.",
    )
    parser.add_argument(
        "--persona",
    ...
    parser.add_argument(
        "--show_chunks",
        action="store_true",
        help="Print retrieved chunks/context before LLM answer",
    )
