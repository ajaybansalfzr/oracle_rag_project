...
# --- Page Configuration ---
st.set_page_config(page_title="Oracle RAG Assistant", layout="wide", initial_sidebar_state="expanded")
...
def parse_rag_output(stdout: str) -> tuple[str, list[str]]:
    ...
    try:
        ...
        if answer_start_index == -1:
            return "Could not parse answer (missing start marker).", []
        sources_start_index = stdout.find(sources_marker)
        if sources_start_index == -1:
            return "Could not parse answer (missing end marker).", []
...
with st.sidebar:
    ...
    if uploaded_file:
        if st.button(
            "Process Document",
            disabled=(st.session_state.process_info["process"] is not None),
        ):
            ...
            with st.spinner("Starting extraction..."):
                ...
                st.session_state.process_info["process"] = subprocess.Popen(
                    cmd,
...
with tab1:
    ...
    with col1:
        ...
        query = st.text_area(
            "Enter your question:",
            height=150,
            placeholder="e.g., How do I configure Smart View for EPM Cloud?",
        )
        ...
        if st.button(
            ...
            if query:
                if not (EMBEDDINGS_DIR / "oracle_index.faiss").exists():
                    ...
                    st.session_state.rag_answer, st.session_state.rag_sources = (
                        "",
                        [],
                    )  # Clear old answer
                    cmd = [
                        python_executable,
                        "-m",
                        "scripts.rag_query_cli_thinking",
                        "--query",
                        query,
                        "--persona",
                        persona.lower(),
