...
def load_resources():
    ...
    try:
        ...
        for model_name in config.EMBEDDING_MODELS:
            ...
            MODELS[model_name]["chunk_meta_conn"] = sqlite3.connect(
            ...
            MODELS[model_name]["section_meta_conn"] = sqlite3.connect(
...
def hierarchical_search(query: str, ensemble: bool = True) -> List[int]:
    ...
    for i, q in enumerate(expanded_queries):
        ...
        for model_name in models_to_search:
            ...
            for section_id, rank in faiss_ranks.items():
                fused_section_scores[section_id] = fused_section_scores.get(section_id, 0) + (
                    1 / (config.RRF_K + rank + 1)
                )
    ...
    candidate_chunk_ids = {
        row["chunk_id"] for row in main_db_conn.execute(relevant_chunks_query, section_ids_as_py_int).fetchall()
    }
    main_db_conn.close()
    ...
    for model_name in models_to_search:
        ...
        cursor.execute(
            f"SELECT chunk_id, summary FROM metadata WHERE chunk_id IN ({placeholders})",
        ...
        if final_vectors.size == 0:
            ...
            _, temp_indices = temp_index.search(
                original_query_vec,
                min(len(final_ids), config.HIERARCHICAL_SEARCH_TOP_N_CHUNKS),
            )
...
def rerank_and_format_context(query: str, chunk_ids: List[int], top_k: int) -> Tuple[str, List[str]]:
    ...
    id_to_text_map = {
        row["chunk_id"]: row["raw_text"]
        for row in meta_conn.execute(
            f"SELECT chunk_id, raw_text FROM metadata WHERE chunk_id IN ({placeholders})",
            tuple(chunk_ids),
        ).fetchall()
    }
    ...
    final_rows = meta_conn.execute(
        f"SELECT * FROM metadata WHERE chunk_id IN ({placeholders_top})",
        tuple(top_chunk_ids),
    ).fetchall()
