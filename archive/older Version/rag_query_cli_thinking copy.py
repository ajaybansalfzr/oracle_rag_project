...
def read_prompt_template(name: str) -> str:
    ...
    try:
        ...
        return "Based ONLY on the context, answer the question.\n\nContext:\n{context}\n\nQuestion:\n{question}"
...
def hybrid_search(query: str, top_n: int = 20) -> list[int]:
    ...
    if chunk_ids_bm25:
        ...
        c.execute(
            f"SELECT chunk_id, vector_id FROM metadata WHERE chunk_id IN ({placeholders})",
            chunk_ids_bm25,
        )
    ...
    for r, v_id in enumerate(vector_ids_faiss):
        if v_id != -1:
            fused_scores[v_id] = fused_scores.get(v_id, 0) + (0.3 * (1 / (k + r + 1)))
...
def rerank_and_format_context(query: str, vector_ids: list, top_k: int) -> tuple[str, list]:
    ...
    c.execute(
        f"SELECT chunk_id, chunk_text FROM metadata WHERE vector_id IN ({placeholders})",
        vector_ids,
    )
    rows = c.fetchall()
    if not rows:
    ...
    c.execute(
        f"SELECT * FROM metadata WHERE chunk_id IN ({placeholders}) ORDER BY CASE chunk_id {order_clause} END",
