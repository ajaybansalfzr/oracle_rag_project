...
def save_to_csv_sqlite(rows: list, csv_path: str, sqlite_path: str):
    fields = [
        ...
        "Image_Refs",
        "Hyperlinks",
        "Spans_With_Fonts",  # <-- Add this if you added it to rows
    ]
    ...
    c.execute(
        ...
        CREATE TABLE IF NOT EXISTS rag_chunks (
            ...
            Spans_With_Fonts TEXT  -- <-- Add this column
        )
    """
    )
    ins = "INSERT INTO rag_chunks VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"  # <-- 14 placeholders
    for r in rows:
...
def generate_context_id(page, region, font_size, is_bold, text):
    short = clean_text(text)[:15].replace(" ", "_")
    bold_flag = "B" if is_bold else "N"
    return f"{page}_{region}_{int(font_size)}_{bold_flag}_{short}"
