...
# This makes the script portable. It finds the project root directory.
...
def log_error(doc_name: str, message: str):
    log_file = LOG_DIR / f"Oracle_Data_{doc_name}.log"  # Use Path object
...
def extract_pdf_blocks(pdf_path: str) -> dict:
    ...
    pages = {}
    doc_name_base = Path(pdf_path).stem
    for i in range(len(doc)):
        try:
            ...
            try:
                ...
                annots = []
                log_error(doc_name_base, f"Page {i+1} annotation error: {str(e)}")
            ...
            log_error(doc_name_base, f"Page {i+1} skipped: {str(e)}")
    doc.close()
    return pages
...
def detect_chapter_from_font(lines: list) -> str:
    ...
    for _, spans in lines:
        for span in spans:
            if span.get("size", 24) >= 24 and "bold" in span.get("font", "").lower():
    ...
    for block in blocks:
        for line in block.get("lines", []):
            for span in line["spans"]:
                if (
                    int(span.get("size", 0)) == 21
                    and span.get("color") == 4616518
                    and "bold" not in span.get("font", "").lower()
                ):
...
def detect_subsection_from_font(lines: list) -> str:
    for _, spans in lines:
        for span in spans:
            if (
                int(span.get("size", 0)) == 16
                and span.get("color") == 2434341
                and "bold" not in span.get("font", "").lower()
            ):
                return clean_text(span.get("text", ""))
    return ""
...
def identify_context_type(text: str) -> str:
    prompt = (
        "Choose exactly one: Overview, Instruction, Definition, Note, Tips, None. Reply with only that word.\n\n" + text
    )
    resp = OLLAMA.chat(model="llama3", messages=[{"role": "user", "content": prompt}])
...
def save_to_csv_sqlite(rows: list, csv_path: str, db_name: str):
    fields = [
        "Document_Name",
        "Chapter",
        "Section",
    ...
    # Save to SQLite
    # NOTE: The original script created a separate DB for each file.
    # We will keep this behavior, though a single DB might be better long-term.
    db_path = Path(csv_path).parent / f"{db_name}.db"
    ...
    ins = f"INSERT INTO rag_chunks ({', '.join(fields)}) VALUES ({', '.join(['?']*len(fields))})"
    # Convert list of dicts to list of tuples for executemany
    rows_to_insert = [[row.get(field, "") for field in fields] for row in rows]
    c.executemany(ins, rows_to_insert)
    conn.commit()
    conn.close()
